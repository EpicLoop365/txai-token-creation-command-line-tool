 ...
 * Each pass has:
 * - Tier level (1-3)
 * - Transfer mode (soulbound / whitelisted / open)
 * - Optional expiration (24h to lifetime)
 * - On-chain metadata with expiry timestamp
 *
 * @example
 * ```typescript
 * import { Pass, PassTier } from '@txai/agent-sdk';
 *
 * // Check a wallet's current pass
 * const tier = await Pass.check('testcore1abc...', 'testnet');
 * console.log(tier); // { level: 1, name: 'Scout', expired: false, daysLeft: Infinity }
 *
 * // Mint a pass via the TXAI API
 * const result = await Pass.mint({
 *   apiUrl: 'https://txai-api.up.railway.app',
 *   wallet: 'testcore1abc...',
 *   tier: 'creator',
 *   durationDays: 30,
 * });
